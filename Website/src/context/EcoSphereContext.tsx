...
export const EcoSphereProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateNotifications = useCallback((value: React.SetStateAction<AppNotification[]>) => {
  ...
  const triggerRewardRedemption = useCallback(
    (rewardId: string): { success: boolean; message: string } => {
      ...
      setCurrentEmployee((prev) => ({ ...prev, points: newPoints }));
      setRewards((prev) =>
        prev.map((r) =>
          r.id === rewardId
      ...
      Promise.all([
        fetch(`/api/rewards/${rewardId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({}) }),
        fetch(`/api/employees/${currentEmployee.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ points: newPoints }) }),
        fetch("/api/notifications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: "Reward Redeemed!", description: `Redeemed: ${targetReward.name}. Deducted ${targetReward.pointsRequired} pts.`, category: "Gamification" }) }),
      ]).then(() => fetchAllData()).catch(console.error);

      return { success: true, message: `Redeemed ${targetReward.name} successfully!` };
    },
    [rewards, currentEmployee, fetchAllData]
  );
  ...
  const triggerJoinChallenge = useCallback(
    (challengeId: string) => {
      ...
      setCurrentEmployee((prev) => ({ ...prev, joinedChallenges: newJoined }));
      setChallenges((prev) =>
        prev.map((c) => (c.id === challengeId ? { ...c, progress: Math.min(c.progress + 10, 100) } : c))
      );

      Promise.all([
        fetch(`/api/employees/${currentEmployee.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ joinedChallenges: newJoined }) }),
        fetch(`/api/challenges/${challengeId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ progress: Math.min(targetChallenge.progress + 10, 100) }) }),
        fetch("/api/notifications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: "Joined New Challenge!", description: `You joined: ${targetChallenge.name}. Earn ${targetChallenge.pointsReward} pts!`, category: "Gamification" }) }),
      ]).then(() => fetchAllData()).catch(console.error);
    },
    [challenges, currentEmployee, fetchAllData]
  );
  ...
  const triggerJoinCSRActivity = useCallback(
    (activityId: string, hasProof: boolean, proofFileName?: string) => {
      ...
      setEmployeeParticipation((prev) => [newPart, ...prev]);

      Promise.all([
        fetch(`/api/csr-activities/${activityId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ joinedEmployees: newJoinedEmployees }) }),
        fetch("/api/employee-participation", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ employeeName: currentEmployee.name, activityName: targetActivity.name, date: newPart.date, hasProof, proofFileName }) }),
        fetch("/api/notifications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: "CSR Participation Logged", description: `Logged for: ${targetActivity.name}. Pending review.`, category: "S" }) }),
      ]).then(() => fetchAllData()).catch(console.error);
    },
    [csrActivities, currentEmployee, fetchAllData]
  );
