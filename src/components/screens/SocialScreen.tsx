...
export const SocialScreen: React.FC = () => {
  ...
  const COLORS = ["#3E8E7E", "#2E7D32", "#6A1B9A", "#C98A2C"];
      ...
      {currentSubTab === "CSR Activities" && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="font-display text-base font-semibold text-text-primary tracking-tight">Corporate Social Responsibility Activities</h3>
            <button
              onClick={() => setShowActivityModal(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 font-semibold text-xs rounded transition-colors uppercase font-mono tracking-wider text-white hover:opacity-90"
              style={{ backgroundColor: "#3E8E7E" }}
            >
              ...
              return (
                <AsymmetricCard key={act.id} module="S" className="flex flex-col justify-between h-48">
                  <div>
                    ...
                    <h4 className="font-display text-sm font-semibold text-text-primary mt-3 leading-snug">
                      {act.name}
                    </h4>
                  </div>
      ...
      {currentSubTab === "Diversity Dashboard" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <AsymmetricCard module="S" className="flex flex-col h-96">
            <h3 className="font-display text-base font-semibold text-text-primary mb-1 tracking-tight">Gender Demographics</h3>
          ...
          <AsymmetricCard module="S" className="flex flex-col h-96">
            <h3 className="font-display text-base font-semibold text-text-primary mb-1 tracking-tight">Diversity & Ethics Training Completion</h3>
              ...
              {mounted ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={trainingCompletionData} margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
                    <CartesianGrid stroke="#E3E8DE" strokeDasharray="3 3" vertical={false} />
                    <XAxis dataKey="name" stroke="#6B7A6E" fontSize={9} axisLine={false} tickLine={false} tick={{ fontFamily: "'Switzer', sans-serif" }} />
                    <YAxis stroke="#6B7A6E" fontSize={9} axisLine={false} tickLine={false} domain={[0, 100]} tick={{ fontFamily: "'JetBrains Mono', monospace" }} />
                    <Tooltip
                      contentStyle={{
                        backgroundColor: "#FFFFFF",
                        borderColor: "#E3E8DE",
                    ...
                    />
                    <Bar dataKey="rate" fill="#3E8E7E" radius={[4, 4, 0, 0]} barSize={15}>
