...
export default function EcoSphereWorkspace() {
  ...
  const [hasInitializedToasts, setHasInitializedToasts] = useState(false);

  // Bridge in-app notifications state to slide-out Toasts
  useEffect(() => {
    // Wait for the notifications to load from the database
    if (notifications.length === 0) return;
    
    if (!hasInitializedToasts) {
      // Mark all pre-existing notifications as toasted on startup so they don't fire
      setToastedIds(notifications.map((n) => n.id));
      setHasInitializedToasts(true);
      return;
    }

    const unread = notifications.filter((n) => !n.read && !toastedIds.includes(n.id));
    ...
    setToastedIds((prev) => [...prev, ...unread.map((n) => n.id)]);
  }, [notifications, toastedIds, hasInitializedToasts]);
